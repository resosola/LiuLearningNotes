...
* 排序保证了重复数在邻近位置出现便于去重，也保证了首尾指针指向的数和目标的比较和指针的移动
...
* 使用**双指针**在原链表直接进行操作，按照奇偶节点拆分为两个链表，然后进行合并，指针修改过程如下：
...
* 同样使用快慢指针，快指针先走k步【保证距离K即可】，然后快慢指针一起走，当快指针走到null时慢指针就到了倒数第k个节点。
...
* 递归，后序遍历实现，递归实现链表遍历，**函数退栈的过程即对应从尾节点向头节点前进的过程**，可以在外部记录一个变量，初始为0，每次函数退栈时变量加加，当变量小于k时还未找到目标节点故返回头节点，大于等于时返回子递归的结果即可。
...
class Solution {
public:
    int getMiddle(vector<int>& nums, int left, int right) {
        // 随机选取分界值
    ...
    void quickSort(vector<int>& nums, int left, int right) {
        ...
        int lRight = middle, rLeft = middle;
        // 针对重复值的优化：与分界值相同的数的排序后位置只会在附近 可缩小待排序部分
        while(lRight > 0 && nums[lRight] == nums[lRight - 1]) --lRight;
        while(rLeft + 1 < nums.size() && nums[rLeft] == nums[rLeft + 1]) ++rLeft;
        // 递归缩小范围的左右区域
        quickSort(nums, left, lRight - 1);
        quickSort(nums, rLeft + 1, right);
    }
...
#### 归并统计
* 归并过程中**因为数组的部分有序性**，我们可以在归并过程中利用这个性质，去统计逆序对的个数
BM20 数组中的逆序对
* 关键点在于合并环节，在合并数组的时候，当发现右边的小于左边的时候，此时可以直接求出当前产生的逆序对的个数。
```c++
#include <vector>
class Solution {
public:
    ...
    void merge(vector<int>& nums, int left, int middle, int right) {
        ...
        int cur = 0;
        int leftSta = left;
        int rightSta = middle + 1;
        while(leftSta <= middle && rightSta <= right) {
            if(nums[leftSta] > nums[rightSta]) {
                 // nums[i] > nums[j]，说明 nums[i] 到 nums[middle] 都与 nums[j] 构成逆序对
                // 记录个数 
                total += middle + 1 - leftSta;
                total %= 1000000007;
                temp[cur++] = nums[rightSta++];
            }else{
                temp[cur++] = nums[leftSta++];
            }
        }
        while(leftSta <= middle) {
            temp[cur++] = nums[leftSta++];
        }
        while(rightSta <= right) {
            temp[cur++] = nums[rightSta++];
        }
        // 将 temp 数组复制回 nums
        for(auto num : temp) {
    ...
    void mergeSort(vector<int>& nums, int left, int right) {
        if(left >= right) return;
        int middle = left + (right - left) / 2;
        mergeSort(nums, left, middle);// 递归左半部分
        mergeSort(nums, middle + 1, right); // 递归右半部分
        merge(nums, left, middle, right);// 合并并计算逆序对
    }
    /**
     * 代码中的类名、方法名、参数名已经指定，请勿修改，直接返回方法规定的值即可
     *
...
public:
    ...
    vector<int> MySort(vector<int>& arr) {
        ...
        for(int i = 0; i < arr.size(); ++i) {
            // 从arr[0]到arr[len - i - 1] 中取最大数放到arr[len - i - 1]上
...
public:
    ...
    vector<int> MySort(vector<int>& arr) {
        // write code here
        for(int i = 0; i < arr.size(); ++i) {
            // 每轮从无序序列[i, arr.size - 1]中选择最小数放到i
            int min = i;
...
最好是O(n)【**基本有序的情况下插入排序的速度非常快**，**第二层循环提前终止**】
...
* 堆排序：
  ...
  class Solution {
  public:
      // 调整堆结构符合大顶堆 从index开始自顶向下调整，堆大小为n
      void Sink(vector<int>& arr, int index, int n) {
          int left = 2 * index + 1;
          while(left < n) {
              int maxChild = left + 1 < n ? (arr[left + 1] > arr[left] ? left + 1 : left) : left;
      ...
      vector<int> MySort(vector<int>& arr) {
          ...
          for(int index = arr.size() / 2 - 1; index >= 0; --index) {
              ...
              Sink(arr, index, arr.size());
          }
          for(int cur =  arr.size() - 1; cur > 0; --cur) {
              swap(arr[cur], arr[0]); // 将堆顶元素与末尾元素进行交换
              Sink(arr, 0, cur); //重新对堆进行调整
          }
          return arr;
      }
  };
...
* ```c++
  ...
  public:
      ...
      // 插入时维持二者结构：数目相差不超过1，左侧元素个数>=右侧元素个数 ,左侧所有数不大于右侧
      void Insert(int num) {
          // 偶数加入大顶堆【保证数目相差】
...
* 对于待排序序列，获取元素的上限值和下限值，创建一个统计数组，数组长度为元素值的范围长度【即上限值-下限值+1】，然后**遍历序列统计对应值的元素个数**，最后**取出统计数组中对应个数的值**即可。
...
* 考虑因素：
  * 待排序的元素个数n
  * 排序稳定性的要求
* 对于不同的应用场景，如何选择合适的排序算法
  * n较大时 采用时间复杂度更低的快排、归并排序或者堆排序
    * 如果还要求空间复杂度尽量低，则适用堆排序
    * 如果要求排序稳定性且空间要求尽量低，则适用归并排序
  * n较小时，因为**统计算法复杂度的大O时一个趋势，不是一个精确的时间**，当n较小时，插入排序等常规算法的**实际执行次数**比快排等时间复杂度更低的算法**更少**，故可选用O(n^2)的算法
    * 对于n较小的情况，**有更高的概率有序**，故选用插入排序是非常值得的。
...
* 常用的排列思路：递归+回溯+剪枝【每层递归确定一个待排位置上的数，回溯用于回到上一层位置重新确定数，剪枝用于去重】
...
* ```c++
  ...
  public:
      vector<string> res;
      // curIndex：当前待确定的位置索引
      void StringSwap(string& cur, int curIndex) {
          // 所有位置都确定完毕
          if(curIndex == cur.length() - 1) {
              res.push_back(cur);
          }
          
          unordered_set<char> cache; // 记录当前待交换的位置已出现过的字母，避免出现重复的排列结果
          // 与后面的数交换来确定字母
          for(int i = curIndex; i < cur.length(); ++i) {
              // 之前已出现重复的字母 当前位置不可再用该相同的字母 否则会有重复排列结果
              if(i > curIndex && cache.find(cur[i]) != cache.end()) continue;
              // 交换
              swap(cur[curIndex], cur[i]);
              StringSwap(cur, curIndex + 1);
              // 回溯
              swap(cur[curIndex], cur[i]);
              cache.insert(cur[i]);
          }
...
* ```c++
  ...
  private:
      vector<string> res;
  public:
      // 当前正在取第length段，开始取的位置为start，新段将加入cur
      void backtrace(string &s, int start, int length, string &cur) {
          if(length == 4){
              if(start == s.size()){
                  // 4段已全部取完 最后一段的.需去掉
          ...
          for(int len = 1; len <= 3; ++len) {
              ...
              auto curCase = s.substr(start, len);
              // 输入只包含数字 越界情况去掉
              if(len == 3 && curCase > "255") return;
              cur += curCase;
              cur += '.';
              backtrace(s, start + len, length + 1, cur);
              // 回溯 注意分隔符号 . 的长度也要算进去
              cur = cur.substr(0, cur.size() - len - 1);
          }
      }
...
* ```java
  ...
  public:
      int rob(vector<int>& nums) {
          ...
          // dp[i]：从i处开始偷所能偷窃到的最高金额
          vector<int> dp(len, 0);
          // 初始化
          dp[len - 1] = nums[len - 1];
          dp[len - 2] = max(nums[len - 2], dp[len - 1]);
          // 从后往前遍历
...
* 动态规划解法
  ...
  public:
      string longestPalindrome(string s) {
          ...
          int start = 0;
          int maxLen = 1;
          vector<vector<int>> dp(n, vector<int>(n, 0)); // dp[i][j]：s[i..j]中最长回文子串
          ...
          for(int i = n - 1; i >= 0; --i){
              // j >= i
              for(int j = i + 1; j < n; ++j){
                  if(s[i] == s[j] && dp[i+1][j-1] == j-i-1){ // j < i 时 dp[i][j] = 0
                      dp[i][j] = j - i + 1; // s[i..j]可构成回文的情况
                      if(maxLen < dp[i][j]){ 
                          maxLen = dp[i][j];
                          start = i;
                      }
...
* ```java
  ...
  public:
      int lengthOfLIS(vector<int>& nums) {
          ...
          int n = nums.size();
          if(n == 1) return res;
          vector<int> dp(n , 1); // dp[i]: 以nums[i]结尾的最长严格递增子序列的长度 隐含初始化dp[0] = 1
          for(int i = 1; i < n; ++i){
              for(int j = i - 1; j >= 0; --j){
                  // 往前遍历找可构成以nums[i]结尾的递增子序列
                  if(nums[j] < nums[i]){
                      dp[i] = max(dp[j] + 1, dp[i]);
                  }
                  res = max(dp[i], res);
...
* ````c++
  ...
  public:
      int longestCommonSubsequence(string text1, string text2) {
          int n = text1.size();
          int m = text2.size();
          // dp[i][j]: text1前i+1个字符和text2前j+1个字符的最长公共子序列长度
          ...
          for (int i = 1; i < n; ++i) {
              for (int j = 1; j < m; ++j) {
                  // dp[i][j] = dp[i-1][j-1] + 1 else dp[i][j] = max(dp[i-1][j],
                  // dp[i][j-1])
                  dp[i][j] =
...
* 大数据量查找
  * 在给定的一台4G的PC机器上实现，一个包含40亿个不重复并且没有排过序的无符号的int整数，给出一个整数，找出给定的某个数 m,是否在文件40亿个数据当中的需求。https://blog.csdn.net/Qgf_666/article/details/88580919
* 海量数据排序
  * 前k个数排序，维持元素个数为k的堆
...
public:
    int myAtoi(string s) {
        ...
        while(index < n && isdigit(s[index])) {
            int digit = s[index] - '0';
            // 判断是否溢出 【转换公式，避免表达式溢出，注：-2147483648也会被判断成溢出，但INT_MAX和INT_MIN只差1故不影响结果】
...
* **循环右移相当于从第*m*个位置开始，左右两部分视作整体翻转**。即abcdefg右移3位efgabcd可以看成AB翻转成BA（这里小写字母看成数组元素，大写字母看成整体）。既然是翻转我们就可以用到reverse函数。![alt](../img/3E6A48137367D997F49AB13EF302653A.gif)
      ...
      int compareVersion(string version1, string version2) {
          ...
          while(index1 < version1.size() || index2 < version2.size()){
              ...
              while(index1 < version1.size() && version1[index1] != '.'){
                  // warning: 需加括号保证(version1[index1] - '0')先执行，因为cur1 * 10 + version1[index1]先执行可能导致整型越界
                  cur1 = cur1 * 10 + (version1[index1] - '0');
                  ++index1;
              }
              while(index2 < version2.size() && version2[index2] != '.'){
                  cur2 = cur2 * 10 + (version2[index2] - '0');
                  ++index2;
              }
              // 比较修订号
...
* C++
  ...
  public:
      ...
      // 修改节点为头节点【最近使用】
      void changePosi(Node* cur) {
          if(cur->pre != nullptr && cur->next != nullptr) removeNode(cur);
          cur->pre = dummyHead;
...
public:
 ...
 if(map.find(key) != map.end()){
    ...
    cache.erase(map[key]);
    cache.emplace_front(key, value); // 原地构造 传入参数进行移动构造 【需模板支持移动构造】
    map[key] = cache.begin();
    return value;
 }
 return -1;
 ...
 if(map.find(key) != map.end()){
    auto iter = map[key];
    cache.erase(iter);// list为双向链表 通过迭代器删除为O(1)时间复杂度
 }else{
    if(cache.size() == cap){
...
* 1、使用优先队列排序，然后取出节点
  ...
  public:
      ...
      ListNode* sortInList(ListNode* head) {
          // write code here
          // 因为自定义了排序规则 所以需要填三个泛型 优先队列更其他容器排序相反，其他容器less时优先级高的是放尾部，不像大顶堆放的是头部
          priority_queue<ListNode*, vector<ListNode*>, compare> pq;
          while(head){
              pq.push(head);
              head = head->next;
    ...
    public:
        ...
        */
        ...
        ListNode* sortInList(ListNode* head) {
            ...
            // 递归左右两边进行排序
            ListNode* leftSortList = sortInList(head);
            ListNode* rightSortList = sortInList(right);
            
            // 合并
            ListNode* dummyHead = new ListNode(-1);
            ListNode* tail = dummyHead;
            while(leftSortList != nullptr && rightSortList != nullptr) {
                // 循环比对
                if(leftSortList->val < rightSortList->val) {
                    tail->next = leftSortList;
                    leftSortList = leftSortList->next;
                }else{
                    tail->next = rightSortList;
                    rightSortList = rightSortList->next;
                }
                tail = tail->next;
            }
            tail->next = leftSortList == nullptr ? rightSortList : leftSortList;
            return dummyHead->next;
        }
    };
  ```
...
* 判断当前待加入序列是否需为相反，如果相反则加入对应逆向迭代器
    ...
    vector<vector<int>> zigzagLevelOrder(TreeNode* root) {
        ...
        while(!q.empty()){ 
            ...
            if(flag) {
                ...
                res.emplace_back(vector<int>{temp.crbegin(), temp.crend()});
            }
            flag = !flag;
        }
        return res;
    ...
    public:
        ...
        vector<int> preorderTraversal(TreeNode* root) {
            ...
            while(!stack.empty()){
                ...
                if(cur->left)
                    stack.emplace(cur->left);// 后加入左节点保证先取出左节点        
            }
            return res;
        }
    };
...
* ```java
  ...
  class Solution {
  private:
      unordered_map<int,int> inorderRoot; // nodeVal -> index
      TreeNode* buildTreeByIndex(vector<int>& preorder, vector<int>& inorder, size_t preLeft, size_t preRight, size_t inLeft, size_t inRight) {
          if(preLeft == preRight) {
          ...
          size_t inRootIndex = inorderRoot[preorder[preLeft]];
          TreeNode* root = new TreeNode(inorder[inRootIndex]);
          root->left = buildTreeByIndex(preorder, inorder, preLeft + 1, preLeft + inRootIndex - inLeft, inLeft, inRootIndex - 1);
          root->right = buildTreeByIndex(preorder, inorder, preLeft + inRootIndex - inLeft + 1, preRight, inRootIndex + 1, inRight);
          return root;
      }
  public:
      TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
          for(size_t i = 0; i < inorder.size(); ++i) {
              inorderRoot[inorder[i]] = i;
          }
          return buildTreeByIndex(preorder, inorder, 0, preorder.size() - 1, 0, inorder.size() - 1);
      }
  };
  ```
...
* 数组要么缺失1～n中的某个数字，要么缺失n+1，而数组正好有下标0～n可以对应数字1～n，因此只要数字1～n中某个数字出现，我们就可以将对应下标的值做一个标记，**最后没有被标记的下标就是缺失的值**。【原地哈希，以索引为指向】
...
* 区间重叠问题，一般需先排序，然后再依据左右区间大小比较进行区间合并
...
* ```c++
  ...
  public:
      vector<vector<int>> merge(vector<vector<int>>& intervals) {
          ...
          //     else return a[1] < b[1];
          // });
          // 两个排序相同 vector默认排序方式按照元素大小进行升序排序 元素类型需重载<
          // 左区间升序下右区间升序 排序
          sort(intervals.begin(), intervals.end());
          vector<vector<int>> res;
          res.push_back({intervals[0][0], intervals[0][1]});
          for(int cur = 1; cur < intervals.size(); ++cur){
              if(intervals[cur][0] <= res.back()[1]){ // 存在重叠
                  // 更新出右区间最大值
                  res.back()[1] = max(res.back()[1], intervals[cur][1]);
              }else{
                  // 不存在重叠 直接加入
                  res.push_back({intervals[cur][0], intervals[cur][1]});
              }
          }
          return res;
      }
...
* ```java
  ...
  public:
      int uniquePaths(int m, int n) {
          if(m == 0 || n == 0) return 0;
          // dp[i][j]：从[i][j]到dp[m-1][n-1]有多少条不同路径
...
* 组合数学方法：
  ...
  * 走到终点需要m+n-2步，需要向下走m-1步，可以转化为，**给你m + n - 2个不同的数，随便取m - 1个数，有几种取法。**那么这就是一个**组合问题了**。![image-20240207142245577](../img/image-20240207142245577.png)

    ![image-20240207142310062](../img/image-20240207142310062.png)

  * ```c++
    class Solution {
    public:
        int uniquePaths(int m, int n) {
            long long ans = 1; // long long 避免乘法溢出
