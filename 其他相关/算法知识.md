...
//TODO 递归算法复杂度计算（1）递归树法（2）主定理法 

todo 根据acwing基础标题整理

//最新复习做题进度 做题分析 2 实现strStr()
...
* 两个链表的第一个公共节点
  ...
  * 法一：得出两链表长度距离，让短链表先走对应距离，然后一起走，第一个相遇节点即为所求
...
* 合并多个有序链表时
  * 采用分治解法：类似归并排序
  * 采用优先队列：先将所有头结点加入队列中，然后每次取出节点，将节点合并，且节点还有后续时将下一节点加入队列中。
...
* f=2s  （快指针每次2步，路程刚好2倍）
* f=s+nb (第一次相遇时，刚好多走了n圈）【**一定在slow未走完一圈的情况下相遇**】
...
* 排序算法中快排和归并排序都属于分治算法，分治法的基本思想：将一个难以直接解决的大问题，分割成一些规模较小的相同问题，以便各个击破，分而治之。

#### 原理

* 快排**基本思想**是**每次在无序序列选择一个数作为基准数**（左端或右端，这里为左端），让小于基准数的放基准数的左边，大于基准数的放基准数的右边，**划分完成后序列被分为两个区域**，对两个区域分别进行上述操作，直至排序完成。

* 让数放到对应位置思路：分配两个哨兵(left,right)指向序列首端和末端，**若基准数为左端，则right先动，否则left先动(//TODOwhy)**，这里为right先动，right--直到找到大于基准值的数，交换两个哨兵所指向的元素,然后left++直至找到小于基准值的数，交换两个哨兵所指向的元素，两哨兵继续移动交换直至left==right，此时分配位置完成。

  注：**两次交换保证了left指向的数小于等于基准数，right指向的数大于等于基准数**
...
* 时间复杂度分析：
  ...
  快速排序的每一次遍历，都**将基数摆到了最终位置上**。第一轮遍历排好 1 个基数，第二轮遍历排好 2 个基数（每个区域一个基数，但如果某个区域为空，则此轮只能排好一个基数），第三轮遍历排好 4 个基数（同理，最差的情况下，只能排好一个基数），以此类推**。总遍历次数为 logn～n 次**，**每轮遍历的时间复杂度为 O(n)**，所以很容易分析出快速排序的**时间复杂度为O(nlogn) ～O(n2)，平均时间复杂度为 O(nlogn)。**

* 优化方式：

  对寻找基准点的方式优化：基本的快排是选取第一个或最后一个作为基准点
...
* 每一趟从无序序列中选出最小的一个元素，顺序放在有序序列的尾部，**直到全部待排序的数据元素排完**。
...
* 希尔排序为了加快速度简单地改进了插入排序，交换不相邻的元素以对数组进行局部排序，并最终用插入排序将局部有序的数组排序。
...
* 桶排序是计数排序的扩展版本，计数排序可以看成每个桶只存储相同元素，而桶排序每个桶（桶的类型视情况而定）存储一定范围的元素，通过**映射函数(常实现了间接的桶间排序)**，将待排序数组中的元素映射到各个对应的桶中，对每个桶中的元素进行排序（有可能再使用别的排序算法或是以递回方式继续使用桶排序进行排序，也可能不需要排序，视情况而定），再通过一定的算法，将桶中的数据提出出来并转换成有序数组。
...
2. 将待排序的一组数，分档规入这些子桶，并对同中数据进行操作。
3. 将各个桶中的数据有序的合并起来
...
* 回溯法（backtracking）是**优先搜索**的一种特殊情况，又称为试探法，常用于**需要记录节点状态的深度优先搜索。**通常来说，排列、组合、选择类问题使用回溯法比较方便。
...
* 对次序为1...n的序列中取k个数的组合可以看成

  次序1的数  加上 从次序为2...n的数中取k-1个数的组合

  次序2的数  加上 从次序为3...n的数中取k-1个数的组合

  .....

  n-k+1 加上 从n-k+2...n中取k-1个数的组合  

* 求1  加上 从2...n中取k-1个数的组合解法

  从2...n中取k-1个数的组合

  又可以看成 2 加上 3..n中取k-2个数的组合
...
* 分析搜索的上界进行剪枝
  ...
  * 易知搜索上界和当前还需要选几个数有关，而当前还需要选几个数与已经选了几个数有关。
  * 搜索的上界 + 接下来要选择的元素个数 - 1 = n
  * 得出i<=n-(k-count)+1；

```java
    public List<List<Integer>> combine(int n, int k) {
        List<List<Integer>> list = new ArrayList<>();
        backStracking(list, new ArrayList(), 1, n, k, 0);
        return list;
    }

    /**
     * 递归回溯法找组合
     * @param list 存放所有组合的集合
     * @param temp 存放当前组合
     * @param j 设定当前待查找元素的搜素起点
     * @param n 元素个数 元素数组从1到n
     * @param k 目标个数 
     * @param count 当前组合持有元素个数
     */
    public void backStracking(List<List<Integer>> list, List<Integer> temp, int j, int n, int k, int count) {
        //当前组合元素个数满足需求 添加并返回
        if (count == k) {
        ...
        for (int i = j; i <= n; i++) {
            //终止条件
            if (count == 0 && i > n - k + 1) break;
            temp.add(i);
            //添加下一个元素
            backStracking(list, temp, i + 1, n, k, count + 1);
            //回溯 将刚添加的元素移除，换成其他元素
...
* 个算法经常被用在树上和图上，我们来思考一下这个问题，**如果给你一个连通图上的一个节点，如何才能得到图上所有的节点呢？** 这个思路其实很简单，首先我们知道，我们可以把给定节点和其邻居加入到答案中，但是邻居还有邻居，因此我们还是得继续这个过程，直到把所有的点都找到，这之中我们可能会遇到一种情况就是，我们访问到了之前找到过的点，因此，这里我们还需要一个判重的机制。这里有一点是，每个点只可能找到其邻居，也就是说只会往其周围的点找，一次只向外扩散一格，**解决广度优先搜索问题，我们会使用队列这么一个 FIFO 的数据结构**，这不难理解，先找到的点我们先考虑其邻居。
...
![image-20220106174459400](D:\Typora\typora-user-images\img\img\image-20220106174459400.png)
...
* 一般方向主要分①从子节点向根节点传递信息【通过给树定义状态，在树上(通过递归)进行状态转移】，②根节点向子节点传递 树操作一般利用递归和搜索，如树的遍历等，用dfs编程会比较简单，但往往状态转移方程不好设计，常常比较难(~~主要是我太菜了~~ )，令人头秃。做题步骤一般是：建树、树的遍历、DP。
...
![image-20220107143247767](D:\Typora\typora-user-images\img\img\image-20220107143247767.png)

##### [647. 回文子串](https://leetcode-cn.com/problems/palindromic-substrings/)

* 回文问题常采用双指针做法，但也可使用动态规划解法
...
![image-20220104195459823](D:\Typora\typora-user-images\img\img\image-20220104195459823.png)
...
* 子序列dp数组【重要】：
  ...
  * 区分一维还是二维：
    ...
    【最长递增子序列、最长连续递增序列、最大子数组和】的状态都是求某个位置结尾能有的最值【最长序列长度、最大和】，只需要一个位置，故为一维。

    【最长公共子序列、最长重复子数组、不相交的线】的状态都是需要两个位置，分别指向两个数组或字符串的某个位置，故为二维
...
* ```java
  class Solution {
      public int findLength(int[] nums1, int[] nums2) {
          ...
          // dp[i][j]：以下标i - 1为结尾的A，和以下标j - 1为结尾的B的 最长公共后缀
...
![image-20220101160945016](D:\Typora\typora-user-images\img\img\image-20220101160945016.png)
...
class Solution {
     ...
     * => sum(P) = (target + sum(nums)) / 2     (target + sum(nums)) / 2可能为负，我们将其转为绝对值即可 target越界时该式为负数 可以直接判断去掉
    ...
    public int findTargetSumWays(int[] nums,int target){
        ...
        //         //不取【0..i-1中容量相同的】 或 取【0..i-1中容量少待取数的 (需当且所需容量大于待取数才可取)】
...
![image-20220106172802463](D:\Typora\typora-user-images\img\img\image-20220106172802463.png)
...
* 难点在于状态转移方程：![image-20220104210545606](D:\Typora\typora-user-images\img\img\image-20220104210545606.png)
        ...
        for(int i = 1; i < dp.length; i++){
            for(int j = 1; j< dp[i].length; j++){
                ...
                if(char1 == char2){
                    ...
                    // 那么dp[i-1][j-1]变化到dp[i-1][j]或dp[i][j-1]只可能为k+1步或k-1步，这样[Math.min(dp[i - 1][j], dp[i][j - 1])) + 1] >=k 故只取dp[i-1][j-1]即可
                }else{
                    // 字符不同 可从三个方向的不同状态变化而来【word1中替换、插入、删除】
...
![image-20220103191743598](D:\Typora\typora-user-images\img\img\image-20220103191743598.png)

![image-20220103191808559](D:\Typora\typora-user-images\img\img\image-20220103191808559.png)![image-20220103191833629](D:\Typora\typora-user-images\img\img\image-20220103191833629.png)
...
* 动态规划模板框架解法：
  ...
  ![image-20220103174309493](D:\Typora\typora-user-images\img\img\image-20220103174309493.png)
...
* ```java
  ...
  class Solution {
      ...
      boolean isFound = false;
      public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
          if(root == null || isFound) return null;
...
* 根据当前数字排列查找下一个排列的算法：
  ...
  ![image-20220110130230893](D:\Typora\typora-user-images\img\img\image-20220110130230893.png)
...
* 而路径经过的节点数减一又可以用左右子树最大深度之和进行表示![image-20220128110409096](D:\Typora\typora-user-images\img\img\image-20220128110409096.png)
...
* 图像旋转技巧：**顺时针90度就是先转置再左右镜像，顺时针180度很简单【上下镜像再左右镜像】，顺时针270就是先左右镜像再转置**![image-20220129004119815](D:\Typora\typora-user-images\img\img\image-20220129004119815.png)
...
* 拓扑排序【类似bfs】解法：![image-20220203135854826](D:\Typora\typora-user-images\img\img\image-20220203135854826.png)
...
* 前缀积解法![image-20220206135413598](D:\Typora\typora-user-images\img\img\image-20220206135413598.png)![image-20220206135604053](D:\Typora\typora-user-images\img\img\image-20220206135604053.png)
...
* ![image-20220224234355695](D:\Typora\typora-user-images\img\img\image-20220224234355695.png)
...
* ![image-20220228104745782](D:\Typora\typora-user-images\img\img\image-20220228104745782.png)
